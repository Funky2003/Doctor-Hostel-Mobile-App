...
class RoomNotifier extends ChangeNotifier {
  ...
  Future getAllRooms({required RoomSort roomSort, required SortingSystem sortBy}) async {
    PostgrestResponse allRoomsData = await roomService.getAllRooms();
    List<RoomModel> roomList = (allRoomsData.data as List<dynamic>).map((element) => RoomModel.fromJson(element)).toList();
  ...
  Future getAllAdminRooms({required int uID}) async {
    var data = await roomService.getAdminRoomsOnly(uID: uID);
    return data.data.map((element) => RoomModel.fromJson(element)).toList();
}
