...
import 'package:room_booking/core/service/favourite.service.dart';
...
class FavouriteNotifier extends ChangeNotifier {
  ...
  Future getAllFavourite({required int userId}) async {
    ...
    print("ALL FAVORITES: ${allFavData!.data}");
    return allFavData!.data.map((element) => FavouriteModel.fromJson(element)).toList();
  }

  Future<bool> addToFavourite({required int userId, required int room_id}) async {
    var data = await favoriteService.addToFavourite(userId: userId, room_id: room_id);
    if (data.hasError) {
