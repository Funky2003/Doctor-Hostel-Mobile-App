...
import 'package:room_booking/core/service/authentication.service.dart';
...
class AuthenticationNotifer extends ChangeNotifier {
  ...
  Future<bool> login({
    ...
    try {
      GotrueSessionResponse response = await _authenticationService.login(email: email, password: password);
      if (response.error == null) {
        notifyListeners();
